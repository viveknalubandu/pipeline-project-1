    ...
    stages {
        ...
        stage('unit-tests') {
            steps {
                ...
                sh "mvn test"
                sleep 5
		snDevOpsArtifact(artifactsPayload:"""{"artifacts": [{"name": "devops_pipeline_demo_dev.jar","version": "${version}","semanticVersion": "${semanticVersion}","repositoryName": "devops_pipeline_demo_dev"}],"stageName": "test"}""")            
            }
            post {
                always {
